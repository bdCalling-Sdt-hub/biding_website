...
import { useActiveCodeMutation, useResendCodeMutation, useVerifyCodeMutation } from '../../redux/api/authApis';
import { toast } from 'sonner';
import { useNavigate } from 'react-router-dom';

const VerifyCode = () => {
    const [otp, setOtp] = useState("");
    const [activeCode, { isLoading }] = useVerifyCodeMutation()
    ...
    const handleVerifyOtp = () => {
        ...
        activeCode({ verifyCode: Number(otp), email: email }).unwrap().then((payload) => {
            // (payload)
            if (payload?.success) {
