...
const ProductDetails = () => {
  ...
  useEffect(() => {
    ...
    const filterBidUser = getSingleAuction?.data?.bidBuddyUsers?.filter(
      (item) => profile?.data?._id === item?.user
    );
    setBidBuddyUser(filterBidUser?.[0]);
  }, [getSingleAuction?.data, profile]);
  const combinedDateTime = useMemo(
    () => new Date(`${auction?.activateTime}`),
    [auction?.activateTime]
  );
  const startingDateTime = useMemo(
    () => new Date(`${auction?.startingDateTime}`),
    [auction?.startingDateTime]
  );
  const [timeLeft, setTimeLeft] = useState(calculateTimeLeft(combinedDateTime));
  const [startTime, setStartTime] = useState(
    calculateTimeLeft(startingDateTime)
  );

  const formatTimeLeft = (time) => {
    if (time.days > 0) {
      return `${time.days} day ${String(time.hours).padStart(2, "0")}:${String(
        time.minutes
  ...
  const heightBidderDataFormat = auction?.bidHistory
    ?.slice(-2)
    ?.map((bidder, i) => {
  ...
  useEffect(() => {
    ...
    socket.on("bidHistory", (updatedBidHistory) => {
      ...
      if (updatedBidHistory?.updatedAuction?._id === id) {
        ...
        const filterBidUser =
          updatedBidHistory?.updatedAuction?.bidBuddyUsers?.filter(
            (item) => profile?.data?._id === item?.user
          );
        ...
        <div className="col-span-12 lg:col-span-10 mx-5 lg:mx-0">
          <div className="flex flex-col lg:flex-row justify-between gap-10">
            <div className="w-full">
              <div>
                <img
                  src={auction?.images?.[imageIndex] ?? "default_image_url"}
              ...
              <div className="flex justify-between items-center mt-5 gap-4">
                {[0, 1, 2, 3].map((i) => (
                  ...
                  >
                    <img
                      src={auction?.images?.[i] ?? "default_image_url"}
            ...
            <div className="bg-white py-5 px-8 w-full rounded-md">
              ...
              <div className="flex justify-between py-5">
                ...
                <div className="text-[#338BFF] text-[26px] font-semibold flex justify-end items-center">
                  {auction?.financeAvailable ? (
                    <p
                      style={{
                        color: "#000000",
                      }}
                      className="text-base font-normal mr-3 inline-block"
                    >
                      (finance available)
                    </p>
                  ) : (
              ...
              {auction?.financeAvailable && (
                ...
                >
                  <p>
                    Total Months For Financing:{" "}
                    <span className="text-yellow">
                      {auction?.totalMonthForFinance}
                    </span>
                  </p>
                  <p>
                    Per Month :{" "}
                    <span className="text-yellow">
                      ...
                      {Number(
                        (auction?.bidHistory?.[auction?.bidHistory?.length - 1]
                          ?.bidAmount ?? 0) / auction?.totalMonthForFinance
                      ).toFixed(2)}
                    </span>
                  </p>
                </div>
              ...
              <Table
                columns={columns}
                dataSource={heightBidderDataFormat?.reverse()}
                size="middle"
                pagination={false}
              />
              ...
              {auction?.status === "COMPLETED" ? (
                ...
                auction?.bidHistory?.[auction?.bidHistory?.length - 1]?.user ? (
                  <div className="text-center">
                    <p
                      className="font-semibold text-4xl mt-3"
                      style={{ color: "#338BFF" }}
                    >
                      Congratulations
                    </p>
                ...
                <div>
                  <div className="text-center mt-5">
                    {auction?.status !== "COMPLETED" && (
                      <h1 className="text-[36px] font-medium text-[#338BFF]">
                        {auction?.status === "ACTIVE"
                          ? isLessThanTenSeconds(formatTimeLeft(timeLeft))
                            ? `00:00:0${time <= 0 ? "0" : time}`
                            : formatTimeLeft(timeLeft)
                          : formatTimeLeft(startTime)?.startsWith("-")
                          ? isLessThanTenSeconds(formatTimeLeft(timeLeft))
                            ? `00:00:0${time <= 0 ? "0" : time}`
                            : formatTimeLeft(timeLeft)
                    ...
                    <p>
                      ...
                      isLessThanTenSeconds(formatTimeLeft(timeLeft))
                        ? "Time Left"
                        : "Time Left to start the auction"}
                    </p>
                  </div>
                  ...
                  {bidBuddyUser?.isActive ? (
                    <div className="flex justify-between items-center lg:px-10 mt-5">
                      <button
                        onClick={() => {
                          if (bidBuddyUser?.isActive) {
                            socket.emit("stopBidBuddy", {
                              auctionId: id,
                              userId: profile?.data?._id,
                              totalBids: Number(bidBuddyUser?.availableBids),
                            });
                          }
                        }}
                      ...
                      <p className="text-xl font-medium">
                        Bids Left:{" "}
                        <span style={{ color: "#338BFF" }}>
                          {bidBuddyUser?.availableBids}
                        </span>
                      </p>
                    </div>
                  ) : (
                    ...
                    <div className="flex gap-5 justify-between mt-5 lg:px-10">
                      ...
                      <Button
                        onClick={() => {
                          ...
                          if (bidBuddyUser?.isActive) {
                            socket.emit("add-bids", {
                              auctionId: id,
                              userId: profile?.data?._id,
                              bids: Number(numberOfBids),
                            });
                          } else {
                            socket.emit("activateBidBuddy", {
                              auctionId: id,
                              userId: profile?.data?._id,
                              totalBids: Number(numberOfBids),
                            });
                          }
                        }}
                      >
              ...
              <p className="text-[#585858] pt-5 px-10">
                The Auto Bid Tool makes bidding easy by automating the process
                for you. Enter the number of bids you want, and the tool will
                place them on your behalf until your set limit is reached or the
                timer runs out. The first bid is placed instantly, so you can
                stay in the auction without constantly keeping track.
              </p>
            </div>
          </div>
