...
const AvailableFinancing = () => {
  ...
  useEffect(() => {
    ...
    socket.on("updated-auction", (updatedBidHistory) => {
      Array.isArray(updatedBidHistory?.auction)
        ? setSocketData([...socketData, ...updatedBidHistory?.updatedAuction])
        : setSocketData([...socketData, updatedBidHistory?.updatedAuction]);
    });
    ...
    auctionsData?.data?.map((item) => {
      socketData?.map((data) => {
        if (item?._id === data?._id) {
          ...
          setAuctionData({
            data: perviousData,
            updatedCount: auctionsData?.updatedCount + 1,
          });
        }
      });
    });
  ...
  return (
    <div className="py-10">
      <div className="flex justify-between items-center">
        <HomeContentHeading title={"Available For Finance"} />
        <Link
          to={`/auctions?financeAvailable=true`}
          className="text-yellow flex items-center gap-1 font-medium cursor-pointer"
        >
          See More
          <IoIosArrowForward />
        </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mx-2 md:mx-0">
        {isLoading ? (
          ...
          auctionsData?.data
            ...
            ?.map((item) => <ProductCard key={item?.key} product={item} />)
        )}
      </div>
    </div>
  );
