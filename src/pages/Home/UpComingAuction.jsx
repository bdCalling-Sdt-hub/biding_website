...
const UpComingAuction = () => {
    ...
    const { data: upcomingData , isLoading } = useGetWinnerQuery({ status: "UPCOMING", page })
    return (
        <div>
            ...
            <div className='grid grid-col-1 md:grid-cols-4 gap-5 my-5 px-2 md:px-0'>
                { isLoading ? <div className='flex items-center justify-center col-span-2 md:col-span-4'><Spin size='large'/></div> :  upcomingData?.data?.result?.slice(0, 4)?.map((product, index) => (
